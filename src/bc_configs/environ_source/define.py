    ...
    def load_yaml_from_file(file_path: str) -> None:
        ...
        try:
            ...
            if not isinstance(yaml_secrets, dict):
                logging.warning(
                    f"YAML file '{file_path}' does not contain a top-level dictionary. Skipping."
                )
                return
            ...
            for k, v in yaml_secrets.items():
                if not isinstance(k, str):
                    logging.warning(
                        f"Key '{k}' in YAML file '{file_path}' is not a string. Skipping."
                    )
                    continue
                if k not in os.environ:
                    ...
                    logging.debug(
                        f"Environment variable '{k}' already exists. Skipping loading from YAML file."
                    )
    ...
    if config.need_to_use():
        ...
        if config.has_filled_connect_fields():
            ...
            vault_secrets = vault_client.secrets.kv.v2.read_secret(
                mount_point=config.mount, path=config.path
            )["data"]["data"]
